...
if [ $SENDCOM = YES ]; then
 for fhr in $fhrs ; do 
    ...
    if [ $SENDDBN = YES ]; then
      $SIPHONROOT/bin/dbn_alert MODEL NARRE_GB2 $job $COMOUT/narre.t${cyc}z.mean.grd${grid}.f${fhr}.grib2
      $SIPHONROOT/bin/dbn_alert MODEL NARRE_GB2 $job $COMOUT/narre.t${cyc}z.spread.grd${grid}.f${fhr}.grib2
      $SIPHONROOT/bin/dbn_alert MODEL NARRE_GB2 $job $COMOUT/narre.t${cyc}z.prob.grd${grid}.f${fhr}.grib2
    fi
 done
fi
